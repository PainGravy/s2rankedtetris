meta = {
    name = 'But what if it was tetris',
    version = '0.3.0',
    component_version = '0.3.0',
    description = 'tetris prototype',
    author = 'gravy oh and i stole roaches sign code LOL LOSER',
    unsafe = false,
}
...
--[[
    TETRIS SIGN DEMO
    ----------------
    This file intentionally scraps all ranked/queue/private-room logic.

    What remains:
      - A camp sign.
      - Interacting with the sign opens one menu button: "Open Tetris".
      - Selecting that button launches a clean three-button Tetris prototype drawn on the HUD.

    Integration notes for the future version:
      - Replace draw_cell() with piece textures later.
      - open_tetris(), close_tetris(), and reset_tetris() are the main entry points.
      - The sign/menu code is separate from the Tetris game logic on purpose.
...
-- drops this main.lua into a clean test pack.
...
local BOARD_H = 20
local CELL = 0.032
local BOARD_LEFT = -0.22
local BOARD_TOP = 0.42
...
local function draw_tetris(ctx)
    render_panel(ctx, 0.51, -0.34, 5, 8, 0.88)
    render_text(ctx, 'chud tetris', 0, 0.74, 0.0012, white)
    ...
    render_text(ctx, 'Score: ' .. tostring(tetris.score), 0.24, 0.36 * ratio, 0.0007, white, VANILLA_TEXT_ALIGNMENT.LEFT)
    render_text(ctx, 'Lines: ' .. tostring(tetris.lines), 0.24, 0.31 * ratio, 0.0007, white, VANILLA_TEXT_ALIGNMENT.LEFT)

    render_text(ctx, 'Controls:', 0.24, 0.21 * ratio, 0.00062, white, VANILLA_TEXT_ALIGNMENT.LEFT)
    render_text(ctx, 'Arrow Keys = move', 0.24, 0.12 * ratio, 0.00055, gray, VANILLA_TEXT_ALIGNMENT.LEFT)
    render_text(ctx, 'Whip = rotate', 0.24, 0.08 * ratio, 0.00055, gray, VANILLA_TEXT_ALIGNMENT.LEFT)
    render_text(ctx, 'Down = fast drop', 0.24, 0.04 * ratio, 0.00055, gray, VANILLA_TEXT_ALIGNMENT.LEFT)

    if tetris.game_over then
        render_text(ctx, 'GAME OVER', 0, 0.05 * ratio, 0.0015, white)
        render_text(ctx, 'Whip restarts     Esc closes', 0, -0.02 * ratio, 0.00075, white)
    end
end
...
local function spawn_sign()
    ...
    sign_uid = spawn_entity(ENT_TYPE.ITEM_SPEEDRUN_SIGN, 46, 84, LAYER.FRONT, 0, 0)
